...
main(int argc, char **argv)
{
    ...
    if (my_rank == 0) // qual o meu papel: sou o mestre ou um dos escravos?
    {
        ...
        for (i = 1; i < proc_n; i++)
        {
            ...
            // coloco o resultado no saco na poisição do emissor-1
            saco[status.MPI_SOURCE - 1] = message; // status.MPI_SOURCE contém o ID do processo que enviou a mensagem que foi recebida
        }
